...
//GCanvas Weex
    ...
    bool
    GCanvasWeex::AddPngTexture(const unsigned char *buffer, unsigned int size, int textureGroupId,
...
//for string cmd
    void
    GCanvasWeex::UsePatternRenderPipeline(int textureListId, int textureWidth, int textureHeight,
...
// From the current position, past semicolon or to end
    ...
    std::string GCanvasWeex::exeSyncCmd(int type, const char *&args) {
        ...
        LOG_D("[GCanvasWeex::exeSyncCmd] type=%d,context type=%d,TODATAURL=%d,width=%d,height=%d,args=%s\n",
              type, GetContextType(), TODATAURL, mCanvasContext->mWidth, mCanvasContext->mHeight,
              args);
    ...
    void GCanvasWeex::bindTexture(struct BitmapCmd cmd) {
        ...
        LOG_D("[webgl::exec] glTexImage2D(%d,%d,%d,%d,%d,%d,%d,%d)",
              cmd.target, cmd.level, cmd.interformat, cmd.width, cmd.height, 0, cmd.format, cmd.type);
